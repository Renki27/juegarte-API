...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 04-12-2020 a las 07:03:57
...
DROP PROCEDURE IF EXISTS `INSERT_ACCOUNT_GAME`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_ACCOUNT_GAME` (IN `p_id_game` INT(3), IN `p_username` VARCHAR(25), IN `p_obtained_points` INT(5))  BEGIN
     INSERT INTO account_game (ID_GAME, USERNAME, OBTAINED_POINTS) 
     VALUES (p_id_game, p_username, p_obtained_points);
END$$

DROP PROCEDURE IF EXISTS `INSERT_ACHIEVEMENT`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_ACHIEVEMENT` (IN `p_title` VARCHAR(50), IN `p_description` VARCHAR(100), IN `p_completed_step` INT(5), IN `p_icon` VARCHAR(50))  BEGIN
     INSERT INTO achievement (ACHIEVEMENT_TITLE, ACHIEVEMENT_DESCRIPTION, A_COMPLETED_STEP, ICON) 
     VALUES (p_title, p_description, p_completed_step, p_icon);
END$$
...
DROP PROCEDURE IF EXISTS `SELECT_ACHIEVEMENTS`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECT_ACHIEVEMENTS` ()  BEGIN
...
DROP PROCEDURE IF EXISTS `SELECT_SCRATCH_QUESTIONS`$$
...
DROP PROCEDURE IF EXISTS `SELECT_TRIVIA_QUESTIONS`$$
...
DROP PROCEDURE IF EXISTS `SELECT_TRUE_FALSE_QUESTION`$$
...
DROP PROCEDURE IF EXISTS `SELECT_TRUE_FALSE_QUESTIONS`$$
...
DROP PROCEDURE IF EXISTS `UPDATE_ACCOUNT_POINTS`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_ACCOUNT_POINTS` (IN `p_USERNAME` VARCHAR(25), IN `p_POINTS` INT(5))  UPDATE account SET account.POINTS = p_POINTS WHERE account.USERNAME = p_USERNAME$$
...
(1, 'Trivia', 'Read the text and select the option you think is the right answer.', 30),
(2, 'True or False', 'Read the text and select true or false according to your knowledge', 30),
(3, 'Scratch Game', 'Scratch the image to reveal it little by little. The more you scratch, more you loose. Then when you are sure about the answer select one of the 4 options ', 30);
...
CREATE TABLE `question` (
  ...
  `QUESTION_INFORMATION` varchar(300) COLLATE utf8_unicode_ci NOT NULL,
...
(25, 2, 'Esta famosa obra de Picasso pertenece al movimiento artístico Modernismo', 'Las Señoritas de Avignon es un cuadro del pintor español Pablo Picasso pintado en 1907 al óleo sobre lienzo y sus medidas son 243,9 x 233,7 cm. Se conserva en el Museo de Arte Moderno de Nueva York. Este cuadro, es la referencia clave para hablar del cubismo, del cual el artista español es el máximo', '../uploads/25-LasSeñoritasdeAvignon.jpg'),
(26, 2, 'La artista Tarsila do Amaral pintó la obra “Abaporu”', 'Abaporu (1928) es el cuadro icónico del arte brasileño, pintado por Tarsila do Amaral, es una obra clave que inauguró uno de los movimientos artísticos más importantes en Latinoamérica: la antropofagia. El tamaño del cuerpo, en contraposición a la minúscula cabeza, expresa el trabajo físico en detri', '../uploads/26-Abaporu.jpg'),
...
(35, 3, '¿Cuál es la obra oculta?', 'La bóveda de la Capilla Sixtina es una obra singular de Michelangelo Bonarotti quien realizó esta magna obra sin ser considerado propiamente un pintor. En los nueves recuadros centrales se muestra un programa iconográfico de las historias del Génesis, desde la creación, la caída del hombre, el diluv', '../uploads/35-CapillaSixtina.jpg'),
(36, 3, '¿Cuál es el artista oculto?', 'Salvador Dalí (1904-1989) fue un pintor, escultor y escritor español del siglo XX. Se le considera uno de los máximos representantes del surrealismo. Es conocido por sus impactantes y oníricas imágenes surrealistas. Sus habilidades pictóricas se suelen atribuir a la influencia y admiración por el ar', '../uploads/36-Dali.jpg'),
(37, 3, '¿Cuál es la obra oculta?', 'La Rueda de Bicicleta (1913) es el primer ready–made de Marcel Duchamp, toda una serie de objetos que creó para desafiar las suposiciones que constituyen una obra de arte. Combina dos piezas, una rueda de bicicleta y un taburete, esto cambió la experiencia del espectador y la óptica de lo que es el ', '../uploads/37-RuedaDeBicicleta.jpg'),
(38, 3, '¿Cuál es la obra oculta?', 'La Fontana di Trevi es una de las mayores (con 40 metros de frente) fuentes monumentales del Barroco en Italia. Los trabajos empezaron en 1732 y terminaron en 1762. Las estatuas de la Abundancia y de la Salubridad, en los dos nichos laterales fueron esculpidas por Filippo della Valle. La fuente es u', '../uploads/38-Fontana.jpg'),
...
(9, 1, '¿A cuál época artística pertenece la obra “El nacimiento de Venus”?', 'El período del Renacimiento es para la pintura una auténtica revolución. Los pintores se separan gradualmente de la imagen religiosa para representar el mundo que les rodea. El cuadro El nacimiento de Venus de Sandro Botticelli fue pintado entre 1482 y 1485. Se trata del primer cuadro en tela pintad', '../uploads/9-Renacimiento.jpg', 'Edad Media', 'Renacimiento', 'Barroco', 'Moderna', 'Renacimiento'),
...
(14, 1, '¿Qué nombre recibe esta obra?', 'Se cree que la obra representa a Cecilia Gallerani, la amante de Ludovico Sforza, duque de Milán. La pintura es uno de los cuatro retratos de mujer pintados por Leonardo, siendo los otros tres La Gioconda, el retrato de Ginevra de\' Benci y el de la Belle Ferronière.', '../uploads/14-LaDamadelArmino.jpg', 'Armiño en brazos de una d', 'Dama y armiño', 'La dama del armiño', 'Dama con armiño', 'Dama con armiño'),
...
(32, 3, '¿Cuál es la obra oculta?', 'Impresión, Sol naciente (1872) se trata del cuadro más famoso de Claude Monet pues es el que dio su nombre al movimiento impresionista. La pintura representa el puerto de El Havre, ciudad en la que el artista pasó gran parte de su infancia. Esta pintura al óleo sobre lienzo se encuentra actualmente ', '../uploads/32-ImpresionSolNaciente.jpg', 'Impresión, Sol naciente', 'El estanque de Ninfeas', 'Crepúsculo en Venecia', 'La urraca', 'Impresión, Sol naciente'),
(33, 3, '¿Cuál es el artista oculto?', 'Diego Rivera (1886-1957)​ fue un destacado muralista mexicano de ideología comunista, famoso por plasmar obras de alto contenido político y social en edificios públicos. Formado en la Escuela de Bellas Artes de San Carlos de la capital mexicana, luego estudió por quince años en varios países de Euro', '../uploads/33-DiegoRivera.jpg', 'Francisco Zúñiga', 'Diego Rivera', 'Rufino Tamayo', 'Gerardo Murillo', 'Diego Rivera'),
(34, 3, '¿Cuál es la obra oculta?', 'Terraza de café por la noche (1888) es una pintura de Vincent van Gogh realizada en Arlés, representando el ambiente de una terraza. En esta pintura expresó sus nuevas impresiones de Francia meridional. Ésta es la primera pintura en la cual Vincent utilizó fondos estrellados.', '../uploads/34-TerrazaDeCafePorLaNoche.jpg', 'El café de noche', 'Los olivos', 'Casas en Auvers', 'Terraza de café por la no', 'Terraza de café por la no'),
(35, 3, '¿Cuál es la obra oculta?', 'La bóveda de la Capilla Sixtina es una obra singular de Michelangelo Bonarotti quien realizó esta magna obra sin ser considerado propiamente un pintor. En los nueves recuadros centrales se muestra un programa iconográfico de las historias del Génesis, desde la creación, la caída del hombre, el diluv', '../uploads/35-CapillaSixtina.jpg', 'Capilla Sixtina', 'Santo Entierro', 'Crucifixión de San Pedro', 'Tondo Doni', 'Capilla Sixtina'),
(36, 3, '¿Cuál es el artista oculto?', 'Salvador Dalí (1904-1989) fue un pintor, escultor y escritor español del siglo XX. Se le considera uno de los máximos representantes del surrealismo. Es conocido por sus impactantes y oníricas imágenes surrealistas. Sus habilidades pictóricas se suelen atribuir a la influencia y admiración por el ar', '../uploads/36-Dali.jpg', 'Pablo Picasso', 'Diego Rivera', 'Vincent van Gogh', 'Salvador Dalí', 'Salvador Dalí'),
(37, 3, '¿Cuál es la obra oculta?', 'La Rueda de Bicicleta (1913) es el primer ready–made de Marcel Duchamp, toda una serie de objetos que creó para desafiar las suposiciones que constituyen una obra de arte. Combina dos piezas, una rueda de bicicleta y un taburete, esto cambió la experiencia del espectador y la óptica de lo que es el ', '../uploads/37-RuedaDeBicicleta.jpg', 'La fuente', 'Monociclo', 'Rueda de bicicleta', 'Viaje redondo', 'Rueda de bicicleta'),
(38, 3, '¿Cuál es la obra oculta?', 'La Fontana di Trevi es una de las mayores (con 40 metros de frente) fuentes monumentales del Barroco en Italia. Los trabajos empezaron en 1732 y terminaron en 1762. Las estatuas de la Abundancia y de la Salubridad, en los dos nichos laterales fueron esculpidas por Filippo della Valle. La fuente es u', '../uploads/38-Fontana.jpg', 'Fontana di Trevi', 'Fontana di Crema', 'Fontana di Piazza Colonna', 'Fontana di Nettuno', 'Fontana di Trevi'),
...
(40, 3, '¿Cuál es la obra oculta?', 'Guernica (1937) es un famoso cuadro de Pablo Picasso, cuyo título alude al bombardeo de Guernica, ocurrido el 26 de abril de 1937, durante la guerra civil española. Fue un encargo del director general de Bellas Artes, Josep Renau, a petición del Gobierno de la Segunda República Española para ser exp', '../uploads/40-Guernica.jpg', 'El sueño', 'Guernica', 'La tragedia', 'Picador', 'Guernica');
...
(18, 2, 'El artista Vincent van Gogh realizó a lo largo de su vida 43 autorretratos', 'Este pintor holandés, uno de los principales exponentes del postimpresionismo. Pintó unos 900 cuadros (entre ellos 43 autorretratos y 148 acuarelas) y realizó más de 1600 dibujos. Esta obra en específico tiene como título “Autorretrato con sombrero de fieltro gris” Fue realizada durante la estancia ', '../uploads/18-AutorretratoenSombrerodeFieltro.jpg', 'true'),
(19, 2, 'El genio artista Miguel Ángel tardo 8 años en terminar su magna obra “La Creación de Adán”', 'La creación de Adán es un fresco en la bóveda de la Capilla Sixtina, pintado por Miguel Ángel alrededor del año 1511. Ilustra uno de los nueve episodios del Génesis representados allí por el artista toscano, ​ en el cual Dios le da vida a Adán, el primer hombre. Miguel ángel tardó cuatro años en rea', '../uploads/19-LaCreacióndeAdán.jpg', 'false'),
(20, 2, 'La Piedad es el nombre recibe esta famosa escultura', 'A finales de 1497, el cardenal Jean de Bilhères-Lagraulas, embajador de Francia en la Santa Sede, le pidió a Miguel Ángel que creara una Piedad a gran escala para su tumba. Al año siguiente, Miguel Ángel comenzó a trabajar en la escultura, que talló en un solo bloque de mármol de Carrara. Cuando la ', '../uploads/20-LaPiedad.jpg', 'true'),
(21, 2, ' La escultura “El Pensador” de Rodin pertenece al período artístico Barroco', 'El pensador es una de las esculturas más famosas de Auguste Rodin del periodo artístico Realismo. El escultor concibió esta pieza entre 1881 y 1882 para decorar el tímpano del conjunto escultórico La puerta del Infierno, encargado en 1880 por el Ministerio de Instrucción Pública y Bellas Artes de Fr', '../uploads/no_image.png', 'false'),
(22, 2, 'El artista Leonardo DaVinci pintó esta mundialmente famosa obra', 'La última cena es una pintura mural original de Leonardo da Vinci ejecutada entre 1495 y 1498. Se encuentra en la pared sobre la que se pintó originalmente, en el refectorio del convento dominico de Santa Maria delle Grazie, en Milán (Italia), declarado Patrimonio de la Humanidad por la Unesco en 19', '../uploads/22-LaÚltimaCena.jpg', 'true'),
(23, 2, 'Revolución Nocturna es el nombre que recibe esta famosa obra del período Barroco', 'La ronda de noche o La ronda nocturna es el nombre por el que se conoce comúnmente una de las más famosas obras maestras del pintor neerlandés Rembrandt, pintada entre 1640 y 1642. Este cuadro es una de las joyas de la exposición permanente del Rijksmuseum de Ámsterdam, una pinacoteca (Edificio dest', '../uploads/23-LaRondadeNoche.jpg', 'false'),
